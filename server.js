...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'running',
    message: 'AI Scam Honeypot Backend is live'
  })
})

/**
 * Receive message (Mock Scammer API)
 */
app.post('/detect-scam', async (req, res) => {
  try {
    ...
    // 📤 Structured JSON output (hackathon requirement)
    res.json({
      is_scam: isScam,
      confidence_score: confidence,
      explanation: isScam
...
app.post('/api/mock-scammer', async (req, res) => {
  ...
  await persistToSupabase(userMessage, classification.scamDetected, classification.confidence)
